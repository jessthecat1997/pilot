				...
				<div class="col-md-5">
					<div class="row">
						...
						<div class="form-group">
							...
							<textarea class="form-control" name="address" id="address">{{ $incident->address }}</textarea>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							...
							<div class="input-group">
								...
								<input type="text" class="form-control money"  id = "fine" name = "fine" data-rule-required="true" style = "text-align: right" value = "{{ $incident->fine }}" required >
							</div>
						</div>
						<div class="form-group required">
							<label class="control-label">Description</label>
							<textarea class = "form-control" name="description" id = "description">{{ $incident->description }}</textarea>
						</div>
						<div class="form-group">
...
<script type="text/javascript">
	$(document).ready(function(){
		//Set data
		@if($incident->delivery_id != null) $('#delivery_id').val("{{ $incident->delivery_id }}"); @endif
		@if($incident->cities_id != null) $('#loc_province').val("{{ $city->provinces_id }}"); fill_cities("{{ $city->provinces_id }}"); $('#cities_id').val("{{ $incident->cities_id }}"); @endif
		...
		$(document).on('click','.save-incident', function(e){
			...
			if($('#incident_date').valid() && $('#incident_time').valid() && $('#date_opened').valid() && $('#description').valid()){
				$.ajax({
					type: 'PUT',
					url:  '{{ route("employees.index") }}/{{ $employee->id }}/incidents/{{ $incident->id }}',
					data: {
		...
		$('#incident_date').datetimepicker({
			...
			format:'Y/m/d',
			formatDate:'Y/m/d',
			value: "{{ Carbon\Carbon::parse($incident->incident_date)->format('Y/m/d') }}",
			startDate:	"{{ Carbon\Carbon::parse($incident->incident_date)->format('Y/m/d') }}",
		});
		...
		$('#incident_time').datetimepicker({
			...
			format:'H:i',
			step:5,
			value: "{{ Carbon\Carbon::createFromFormat('H:i:s', $incident->incident_time)->format('H:i') }}",
			startDate:	"{{ Carbon\Carbon::createFromFormat('H:i:s', $incident->incident_time)->format('H:i') }}",
		})
		...
		$('#date_opened').datetimepicker({
			...
			format:'Y/m/d',
			formatDate:'Y/m/d',
			value: "{{ Carbon\Carbon::parse($incident->date_opened)->format('Y/m/d') }}",
			startDate:	"{{ Carbon\Carbon::parse($incident->date_opened)->format('Y/m/d') }}",
		});
		...
		$('#date_closed').datetimepicker({
			...
			value: "{{ Carbon\Carbon::parse($incident->date_closed)->format('Y/m/d') }}",
			startDate:	"{{ Carbon\Carbon::parse($incident->date_closed)->format('Y/m/d') }}",
			@endif
			...
			$.ajax({
				...
				success: function(data){
					if(typeof(data) == "object"){
						...
						$('#cities_id').val("{{ $incident->cities_id }}");
